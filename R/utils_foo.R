...
#'
#' @param base.dir Character; base directory of the project.
#' @param plot.name Character; plot filename stem.
#' @param dir.name Character; name of the destination subdirectory.
#' @param file.names Character vector of file identifiers.
#' @param mainfold Logical; if \code{TRUE}, looks for plots directly inside
#'   each file's results folder. Defaults to \code{FALSE}.
#' @param second_fold Character; optional intermediate subdirectory path.
#'   Defaults to \code{""}.
#' @param channel Character; channel identifier used in plot filenames.
#' @param storage_fold Character; top-level output folder. Defaults to
#'   \code{"plots"}.
#' @param short Logical; if \code{TRUE}, uses \code{shortname} as the
#'   destination filename. Defaults to \code{FALSE}.
#' @param shortname Character; filename used when \code{short = TRUE}.
#'   Defaults to \code{""}.
#' @param ... Currently unused.
#'
#' @return Character vector of destination file paths (invisibly).
#' @keywords internal
pull_plots_old <- function(
  base.dir,
  plot.name,
  dir.name,
  file.names,
  ...
  folder.vec <- sapply(
    file.names,
    function(filename, base.dir, mainfold, second_fold) {
      if (mainfold) {
        file.path(base.dir, paste0(filename, "_results"))
      } else {
        do.call(
          file.path,
          as.list(c(base.dir, paste0(filename, "_results"), second_fold))
        )
      }
    },
    base.dir,
  ...
  plot.names <- sapply(
    file.names,
    function(filename, channel, plot.name, short) {
      if (short) {
        shortname
      } else {
        paste0(filename, "_", channel, "_", plot.name)
      }
    },
    channel,
    plot.name,
  ...
  plots.dir = file.path(base.dir, storage_fold)
  ...
  filepaths = sapply(
    plot.path,
    function(file, plots.dir.second) {
      if (short) {
        # catch which of filenames is in the plots.dir.second
        realname = filenames[sapply(filenames, grepl, x = file)]
        #add filename to prevent overwriting
        destination.path <- file.path(
          plots.dir.second,
          paste0(realname, "_", shortname)
        )
      } else {
  ...
  filepaths <- setNames(filepaths, NULL)
  return(filepaths)
}
